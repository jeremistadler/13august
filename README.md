...
Cookies identifierar webbläsaren, inte en fysisk person. Rensade cookies eller en annan webbläsare kan ge en ny anmälan. Nya anmälningar skickar en kort SMS-sammanfattning via 46elks till arrangörernas två nummer, med avsändaren FEST. Ändringar och upprepade försök för samma session skickar inte nya SMS. Workern använder hemligheterna `ELK_USERNAME` och `ELK_PASSWORD`. SMS skickas i bakgrunden efter sparandet, aldrig från lokal utveckling. Fel loggas utan meddelandetext eller inloggningsuppgifter; de påverkar inte sparandet. Utskicken är best effort utan automatisk återförsökning (för att undvika dubbla SMS vid osäkra nätverksfel). Fullständiga svar finns alltid i adminvyn. Arrangören kan se alla svar på `/admin`. Sidan skyddas med HTTP Basic-inloggning som kontrolleras i Workern innan någon information hämtas. Användarnamn och lösenord lagras som Cloudflare-hemligheterna `ADMIN_USERNAME` och `ADMIN_PASSWORD`, aldrig i klientkod eller Git. Lokalt läggs de i den ignorerade filen `.dev.vars`. Adminsvar får inte cachas eller indexeras och inskickad text HTML-escapas. Ingen databasändring krävs för adminvyn.
